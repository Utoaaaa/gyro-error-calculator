...
export function AstronomyEngineTimeInputForm({ onTimeChange, resetTrigger, reductionSeconds, setReductionSeconds, utcDateTime, setUtcDateTime }: AstronomyEngineTimeInputFormProps & { resetTrigger?: number, reductionSeconds?: string, setReductionSeconds?: (v: string) => void, utcDateTime?: string, setUtcDateTime?: (v: string) => void }) {
  ...
  // 修正：localDate/localTime/timezone/reductionSeconds 變動時自動計算 GHA 查詢時間
  useEffect(() => {
    if (localDate && localTime && timezone && reductionSeconds !== undefined) {
      const utc = getUtcFromLocal(localDate, localTime, timezone);
      const reductionSecNum = reductionSeconds === "" ? 0 : Number(reductionSeconds);
      setGhaQueryTime(getGhaQueryTime(utc, reductionSecNum));
    }
  }, [localDate, localTime, timezone, reductionSeconds]);
  ...
  const setCurrentTime = () => {
    ...
    // 修正：若 reductionSeconds 為空，主動設為 "0"，以確保 GHA 查詢時間能正確顯示
