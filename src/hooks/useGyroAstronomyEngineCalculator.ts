...
export function useGyroAstronomyEngineCalculator() {
  // 緯度狀態
  ...
  // 時間輸入（UTC ISO 字串）
  const [utcIsoString, setUtcIsoString] = useState('');
  // TimeInputForm UTC 欄位顯示用
  const [utcDateTime, setUtcDateTime] = useState('');
  // 減秒示例
  const [reductionSeconds, setReductionSeconds] = useState('0');

  // GHA查詢時間（來自 TimeInputForm）
  const [ghaQueryTime, setGhaQueryTime] = useState<string>("");
  ...
  const handleCalculate = () => {
    try {
      ...
      const inputData = {
        ...
        lonMinutes,
        lonDirection,
        utcIsoString: ghaQueryTime,
        gyroAzimuth
      };

      const results = performCalculationWithAstronomyEngine(inputData);
  ...
  const loadExampleData = () => {
    ...
    setLonMinutes('3.1');
    setLonDirection('E');
    setUtcIsoString('2025-07-05T07:15:24Z');
    setUtcDateTime('2025-07-05T07:15:24');
    setReductionSeconds('38');
  ...
  return {
    ...
    utcIsoString, setUtcIsoString,
    utcDateTime, setUtcDateTime,
    reductionSeconds, setReductionSeconds,
    ghaQueryTime, setGhaQueryTime,
